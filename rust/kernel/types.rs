...
use crate::sync::atomic::{ordering, Atomic, Relaxed};
use core::{
    cell::UnsafeCell,
...
impl<T> AtomicOptionalBoxedPtr<T> {
    ...
    pub fn new(value: Option<KBox<T>>) -> Self {
        Self {
            ptr: Atomic::new(Self::to_ptr(value)),
        }
    }
    ...
    pub fn swap<O: ordering::Ordering>(&self, value: Option<KBox<T>>, order: O) -> Option<KBox<T>> {
        let ptr = self.ptr.xchg(Self::to_ptr(value), order);
        if ptr.is_null() {
            return None;
        }
    ...
    pub fn store<O: ordering::Ordering>(&self, value: Option<KBox<T>>, order: O) {
        self.swap(value, order);
    }

    /// Stores a new value and returns the old one.
    pub fn take<O: ordering::Ordering>(&self, order: O) -> Option<KBox<T>> {
