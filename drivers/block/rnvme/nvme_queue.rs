...
struct NvmeQueueInner<T: mq::Operations<RequestData = NvmeRequest> + Send + 'static> {
    sq_tail: u16,
    last_sq_tail: u16,
    irq: Option<Arc<irq::Registration<Arc<NvmeQueue<T>>>>>,
}

#[pin_data]
pub(crate) struct NvmeQueue<T: mq::Operations<RequestData = NvmeRequest> + Send + 'static> {
    ...
    T: mq::Operations<RequestData = NvmeRequest> + Send,
{
    pub(crate) fn try_new(
        ...
        for i in 0..depth {
            cq.write_item(i.into(), &NvmeCompletion::default());
        }
    ...
    pub(crate) fn submit_command(&self, cmd: &NvmeCommand, is_last: bool) {
        // TODO: irqdisable
        let mut inner = self.inner.lock();
        self.sq.write_item(inner.sq_tail.into(), cmd);
        inner.sq_tail += 1;
    ...
    pub(crate) fn register_irq(self: &Arc<Self>, pci_dev: &pci::Device<device::Bound>) -> Result {
        pr_info!(
            "Registering irq for queue qid: {}, vector {}\n",
            self.qid,
            self.cq_vector
        );
        let handler: Result<Arc<Self>> = Ok(self.clone());
        let reg = Arc::pin_init(
            pci_dev.request_irq_by_index(
                self.cq_vector.into(),
        ...
        // TODO: irqdisable
        self.inner.lock().irq.replace(reg);
        Ok(())
    }
}
...
where
    T: mq::Operations<RequestData = NvmeRequest> + Send + 'static,
{
    fn handle(&self, _device: &Device<Bound>) -> irq::IrqReturn {
...
kernel::impl_transmute_via_zerocopy!(NvmeCompletion);
kernel::impl_transmute_via_zerocopy!(NvmeCommand);
// Only `le<u64>` needs the kernel byte-safety traits (it backs the PRP `dma::Pool`);
// the other `le<N>` are struct fields covered by their container's zerocopy derive.
kernel::impl_transmute_via_zerocopy!(crate::nvme_defs::le<u64>);
