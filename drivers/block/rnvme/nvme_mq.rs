    ...
    T: mq::Operations<RequestData = NvmeRequest> + Send,
{
    match rq.command() {
        ...
        kernel::block::mq::Command::Write | kernel::block::mq::Command::Read => {
            ...
            let mut cmd = NvmeCommand(::zerocopy::transmute!(NvmeRw {
                opcode: opcode as _,
            ...
            if rq.nr_phys_segments() == 1 {
                    ...
                    <= NVME_CTRL_PAGE_SIZE * 2
                {
                    ...
                    if len > (NVME_CTRL_PAGE_SIZE as u32) {
                        cmd.0.prp2 = (dma_addr + (NVME_CTRL_PAGE_SIZE as u64)).into();
                    }
            ...
            let page_count = setup_prps_iter(
                &io_queue.data,
                &mut cmd,
                &mut md,
                &*rq,
            ...
            pdu.first_dma
                .store(cmd.0.prp2.into(), Ordering::Relaxed);
            pdu.mapping_data.store(Some(md), Relaxed);
    ...
    T: mq::Operations<RequestData = NvmeRequest> + Send,
{
    ...
    if let Some(md) = pdu.mapping_data.take(Relaxed) {
        free_prps(
        ...
        // `md` drops here: `RequestDmaMapping::Drop` unmaps the DMA mapping.
    } else {
