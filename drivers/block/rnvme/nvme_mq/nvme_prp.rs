...
/// DMA-map `req` and build `cmd`'s PRP list, storing the RAII mapping in `md`
/// (torn down by its `Drop` in `complete()`, or on error). Returns the number of
/// PRP-list pages allocated.
pub(crate) fn setup_prps_iter<T: Operations>(
    data: &NvmeData,
    cmd: &mut NvmeCommand,
    md: &mut MappingData,
    req: &Request<T>,
    ...
    let mut mapping = req.dma_map(dev, length)?;
    let page_count = build_prps(data, cmd, mapping.segments(), &mut md.pages, length)?;
    md.mapping = Some(mapping);
    Ok(page_count)
}

/// NVMe-specific PRP construction: emit PRP entries into `cmd` from the mapped
/// `segs`, using `pages` for PRP-list allocations. Returns the pages allocated.
fn build_prps(
    data: &NvmeData,
    cmd: &mut NvmeCommand,
    mut segs: Segments<'_>,
    ...
    // `None` (here or later) means a segment was needed but the mapping ended.
    let seg = segs.next().ok_or(EIO)??;
    let page = nvme_driver_defs::NVME_CTRL_PAGE_SIZE;
    let mut dma_addr = seg.addr;
    let mut dma_len = seg.len;
    let offset = dma_addr & ((page - 1) as u64);

    let consumed = ((page as u64) - offset) as u32;

    cmd.0.prp1 = dma_addr.into();
    ...
    if length <= page as u32 {
        cmd.0.prp2 = dma_addr.into();
        return Ok(0);
    }

    let mut prp_list = ManuallyDrop::new(data.dma_pool.try_alloc(true)?);

    cmd.0.prp2 = prp_list.dma_handle().into();
    pages[0] = prp_list.start_ptr() as usize;
    struct Data<'a> {
        page_count: usize,
        pages: &'a mut [usize],
        first_dma: u64,
